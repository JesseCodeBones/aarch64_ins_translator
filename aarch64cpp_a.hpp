...
static uint32_t add_extended_register(uint8_t sf, uint8_t rd, uint8_t rn,
                                      uint8_t rm, uint8_t option, uint8_t imm3);
 ...
 */
static uint32_t add_pc_and_imm_page(uint8_t immlo, uint32_t immhi, uint8_t rd);
...
@param rn First operand register
*/
static uint32_t authenticate_data_address_A(uint8_t z, uint8_t rd, uint8_t rn);
/**
  只能在特权模式下执行，所以不准备有test
...
The address is in the general-purpose register that is specified by <Xd>.
  ...
  @param rn First operand register
  */
static uint32_t authenticate_data_address_B(uint8_t z, uint8_t rd, uint8_t rn);
/**
只能在特权模式下执行，所以不准备有test
